...
We first set up the model for the `Game` which contains the `Player` with the position
...
We declare the type of the message here which can be one of the following:
